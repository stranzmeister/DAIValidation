...
# TODO: Logging level - when it's info it's still reporting warning and error messages
# TODO: Work on removing the following line (around line 1570):
#       journal.metric_expected(swap_event_term(impression))
# TODO: Need to determine what to do with the metrics for the slotImpression (around line 192)
# TODO: Look into using the records from the SQL database as a source to generate metrics from (line 1756)
# TODO: Determine if this function is still needed
#       - def handle_ad_request(match)
# TODO: Need to determine how to handle internal 'slotCompleted' or 'AdBreakCompleteEvents' (line 1805)
# TODO: SM3 - Need to determine a way to validate firing sequence for all beacons.  Seeing an issue where
#       the slotImpression FIRED AFTER the defaultImpression (file:///tmp/Ad_Response_num_03_Slot_ID_1073791454.html)
# TODO: Need to work on removing the populate_template function.

# version 1.8
# Fixed: Issue in the validate_ad_request_url function where qvt['duration'] was being assigned
#        to the tpos value.  And fixed mind, maxd, and tpos so that comparision are being made
#        to determine if the value is true or not.
# Fixed: Missing tracking impression when multiple Ad Ids have use the same tracking impression.
# Fixed: The "Expected Firing Time" has been changed to match the timezone for the log entry.
# Fixed: The time_position in the Validated Ad Request was using the duration variable.
# Fixed: Seekable channels contain multiple slot IDs.
# Fixed: Identify missing impressions in the XML.
# Fixed: The difference between the stop_offset minus the start_offset should be rounded to
#        the nearest value for "MAXD" and "MIND" parameters.
# Fixed: The message in the DAI Validation report so that the beacon firing order for Tracking
#        Impressions reports "Pass" or "Fail".
# Fixed: The message in the DAI Validation report so that the HTTP Status for the Tracking
#        Impression reports "Pass" or "Fail".
#
# version 1.8.1
# Changes to validate_ad_request_url function:
#    Fixed: Added "Air TV Player" as a device for the Channel Service ID (CSID).
#    Fixed: Added "http://b-adapt.movetv.com".
#    Fixed: Convert "MAXD", "MIND", and "TPOS" to integers to strip the decimal place.

# version 1.8.2
# Changes to calculate_difference_in_beacon_firing_times function:
#   removed: delta = round(expected_firing_time - actual_firing_time, 2)
#   added:   delta = round(actual_firing_time - expected_firing_time, 2)
...
class SimpleGrep:
    """ A set of regular expressions used to parse certain Dynamic Ad Insertion events from the AP player.log. """
    ...
    __slots__ = ['qvt_parsed', 'ad_request', 'xml_response', 'sm3_points', 'avg_beacon', 'fw_beacon',
                 'fw_callback', 'ap_internal', 'ap_external', 'ap_response']
...
class ParseAdRequest:
    ...
    The Ad Request submitted to FreeWheel looks like this:
        YYYY/MM/DD HH:MM:SS http://[Id].v.fwmrm.net/ad/g/1?[globalParams];[keyValues];[ParamsForSlot1];[ParamsForSlotN]
    """

    def __init__(self, log):
        self.log = log
        # self.missing = colored('Missing', 'red')
    ...
    def _parse(self):
        ...
        # The Ad Request (URL) parameters contain the following sections:
        expected_params = {
            'flag', 'nw', 'resp', 'prof', 'csid', 'caid', 'ssnw',  # 1) Global info about URL request as a whole
            'asnw', 'vdur', 'pvrn', 'vprn', 'metr', 'afid', 'sfid', 'ssto',
            'emcr', 'esvt', 'exvt', 'rste', 'sync', 'qtcb', 'slcb',  # 2) FreeWheel capabilities
            'cpsq', 'envp', 'slid', 'tpcl', 'ptgt', 'maxd', 'mind',  # 3) Slot info and type of ad request
            'slau', 'tpos',
            'comscore_device', 'comscore_impl_type', 'comscore_platform',  # 4) Site specific values
            'feature', 'module', 'nielsen_dev_group', 'nielsen_os_group',
            'nielsen_os_version', '_fw_nielsen_app_id', 'fw_vcid', '_fw_vcid2',
            'mode', 'vip', 'roku_rida',
        }
        ...
        if self.log:
            self.timestamp = re.match(r'^(?P<date>\d{4}/\d{2}/\d{2}\s\d{2}:\d{2}:\d{2})', self.log).group(1)
            self.url = re.match(r'^.* (?P<url>http.*$)', self.log).group(1)
        else:
        ...
        self.params.update((key, value) for key, value in parse_qs(parsed.query).items())
        # Update the values of some of the expected parameters
        self.params['ads_url'] = parsed.path.strip('/')

        # FreeWheel Capabilities (e.g., exvt qtcb emcr slcb esvt sync rste).
        # self.params['exvt'] = re.search('exvt', str(self.params['flag'])).group(0)
        # self.params['qtcb'] = re.search('qtcb', str(self.params['flag'])).group(0)
        # self.params['esvt'] = re.search('esvt', str(self.params['flag'])).group(0)
        # self.params['slcb'] = re.search('slcb', str(self.params['flag'])).group(0)
        # self.params['emcr'] = re.search('emcr', str(self.params['flag'])).group(0)
        # self.params['sync'] = re.search('sync', str(self.params['flag'])).group(0)
        # self.params['rste'] = re.search('rste', str(self.params['flag'])).group(0)


class Impression:
    """
    Create an object view of the Impression parameters, such as Advertisement Slot ID (e.g., slotImpression),
    Advertisement IDs (e.g., Quartiles), and 3rd-party Tracking Impressions.
    """

    # Map FreeWheel impression terminology to terms found within the Adaptive Player log
    ...
    def __init__(self, source, **kwargs):
        ...
        self.tech_type = None  # server-side (sm3) or client-side (client) ad stitching
        ...
        self.url = None  # contains the beacon's url
        self.utc = None
    ...
    def swap_event_term(self, event):
        """
        Replace the Adaptive Player's terminology (i.e., AdStartEvent) with FreeWheel's terminology (defaultImpression)
        or vice versa.
        """
        if event is None:
            return None
...
class SQLiteDB:
    ...
    def __init__(self, row=False):
        ...
        directory = r'C:\Users\shanil.jain.ECHOSTAR\Documents'
        filename = 'sqlite::memory:?cache=shared'
...
def catalog_qvt(logline):
    # type: (str) -> None
    """
    The Adaptive Player periodically retrieves the QVT and displays the information in the player.log.  Ideally,
    when validating the Adaptive Player, we should retrieve the QVT information from the same source the Adaptive
    Player used, before it expires, and avoid using data displayed by the Adaptive Player whenever possible.

    :param str logline: The Adaptive Player's log line for the QVT.
    :return: None

    Note:
        Encountering cases where, prior to the Ad Request, the QVT within the logfile doesn't contain all of the
        cue_points until after the Ad Request has been submitted to FreeWheel.  In one of these cases, the Adaptive
        Player appears to discarding the first cue_point because it occurred in the past and submitted a request for
        the 2nd cue_point.  The QVT used by the AP wasn't observed until later in the player.log.

        - Schedule QVTs are used for both live and lookback content.
        - Entitlement QVTs are used for SVOD and TVOD
    """

    def compute_duration(stop_offset, start_offset):
        """ Calculate the duration of the Slot Impression in seconds. """
        if all([stop_offset, start_offset]):
            return str(int(round(float(stop_offset) - float(start_offset))))

    def compute_time_position(start_offset, anchor_time):
        """ Calculate the time position. """
        if all([start_offset, anchor_time]):
            return str(int(round(float(start_offset) - float(anchor_time))))

    def update_database(values):
        """ Inserts or Replaces the records in the QVT table in the database. """
        sql_statement = '''
                REPLACE INTO qvt_table
                VALUES (
                    :slot_id, :afid, :asnw, :caid, :channel, :nw, :ssnw, :prof, :emcr, :esvt, :exvt, :qtcb, :slcb, 
                    :tpcl, :anchor_time, :allow_seeking, :method, :start_offset, :stop_offset, :playback_delay, 
                    :start_offset_with_delay, :duration, :time_position, :title, :url, :vod, :log
                    );
                '''
    ...
    try:
        json_data = ''
        # Verify the Adaptive Player log line contains the QVT with the 'Ad Info and 'Ad Break' sections.
        if re.search(r'"ad_info":.*"ad_breaks":', logline):
            ...
            if m:
                ...
                ad_breaks = json_data.get('playback_info', {}).get('ad_info', {}).get('ad_breaks', {})
        else:
            logging.warning('Skipped parsing the QVT because the Ad Break section is absent.')
            return None
        ...
        try:
            # Attempt to parse the QVT for the Dynamic Advertisement related parameters and store them in
            # a dictionary so that we can update the qvt_table in the database using named placeholders.
            missing = 'Missing'
            values = dict()
            values['log'] = logline
            values['playback_delay'] = json_data.get('playback_delay', 0)
            values['url'] = json_data.get('self', missing)
            values['title'] = json_data['shows'][0]['title']
            values['vod'] = json_data['shows'][0]['vod']

            # Ad properties
            values['afid'] = ad_info.get('fallback_asset_id', missing)
            values['asnw'] = ad_info.get('programmer_network_id', missing)
            values['caid'] = ad_info.get('programmer_asset_id', missing)
            values['channel'] = ad_info.get('channel_name', missing)
            values['nw'] = ad_info.get('network_id', missing)
            values['ssnw'] = ad_info.get('network_id', missing)
            values['prof'] = ad_info.get('profile', missing)

            # FreeWheel Capabilities
            values['emcr'] = ad_info.get('fw_capabilities', {}).get('expectedMultipleCreativeRenditions', missing)
            values['esvt'] = ad_info.get('fw_capabilities', {}).get('enableServerVastTranslation', missing)
            values['exvt'] = ad_info.get('fw_capabilities', {}).get('recordVideoView', missing)
            values['qtcb'] = ad_info.get('fw_capabilities', {}).get('requiresQuartileCallbackUrls', missing)
            values['slcb'] = ad_info.get('fw_capabilities', {}).get('supportsSlotCallbacks', missing)

            # Linear properties
            linear_info = json_data.get('playback_info', {}).get('linear_info', {})
            values['anchor_time'] = linear_info.get('anchor_time', '0')
            values['allow_seeking'] = linear_info.get('allow_seeking', missing)

            # Append the provider's name to the profile to match the same parameter from the Ad Request
            if 'Missing' not in values['prof']:
                values['prof'] = ad_info.get('profile', missing).join(['', ':sling'])

            # Extract the proper URL from the QVT's Ads Service URL parameter
            values['ads_url'] = ad_info.get('ads_service_url', missing)
            # FIXME: Python 3.6 is throwing the following exception:
            #  An exception of type TypeError occurred. Arguments: ("a bytes-like object is required, not 'str'",).
            if 'Missing' not in values['ads_url']:
                ads_url = re.match(r'http:.+?/(?P<url>http.*$)', values['ads_url']).groups('url')
                values['ads_url'] = ''.join(ads_url)

            # Condition: Ad Break contains single commercial break.
            if isinstance(ad_breaks, dict):
                values['slot_id'] = ad_breaks.get('cue_point_number')
                values['method'] = ad_breaks.get('method')
                values['tpcl'] = ad_breaks.get('type')
                values['start_offset'] = float(ad_breaks['start_offset'])
                values['stop_offset'] = float(ad_breaks['stop_offset'])
                values['duration'] = compute_duration(values['stop_offset'], values['start_offset'])
                values['time_position'] = compute_time_position(values['start_offset'], values['anchor_time'])
                values['start_offset_with_delay'] = values['start_offset'] + values['playback_delay']
                update_database(values)

            # Condition: Ad Break contains multiple commercial breaks.
            elif isinstance(ad_breaks, list):
                for ad_break in ad_breaks:
                    values['slot_id'] = ad_break.get('cue_point_number')
                    values['method'] = ad_break.get('method')
                    values['tpcl'] = ad_break.get('type')
                    values['start_offset'] = float(ad_break['start_offset'])
                    values['stop_offset'] = float(ad_break['stop_offset'])
                    values['duration'] = compute_duration(values['stop_offset'], values['start_offset'])
                    values['time_position'] = compute_time_position(values['start_offset'], values['anchor_time'])
                    values['start_offset_with_delay'] = values['start_offset'] + values['playback_delay']
                    update_database(values)
...
def process(line):
    ...
    """
    Determine if the log entry from the Adaptive Player matches any significant Dynamic Ad Insertion events.

    :param str line: The log line from the Adaptive Player log
    :param Journal journal: TBD
    """
    try:
        # logging.info(line)
        # if "1210949484" in line:
        #     logging.warning(line)
        # if line.startswith("2019/02/26 22:15:35.753334"):
        #     logging.warning(line)
        ...
        if match.qvt_parsed:
            ...
            # journal.ad_request_store_url(line)
...
def handle_ad_request(match):
    ...
    try:
        line = match.group('logline')
        ad_request = ' '.join([match.group('timestamp'), match.group('url')])
        with SQLiteDB() as cursor:
            cursor.execute('INSERT INTO dai_events_table (ad_request) VALUES (?);', (ad_request,))
    except Exception as ex:
        logging.error('Problem encountered while processing the following FreeWheel Ad Request: {0}.'.format(line))
        logging.error(format_exception(ex))
...
def handle_ad_response(match):
    ...
    def fetch_pod_id():
        """ Retrieves the pod_id of the record containing an Ad Request, but not a corresponding Ad Response. """
        query = 'SELECT pod_id FROM dai_events_table WHERE ad_request IS NOT NULL AND ad_response is NULL;'
        with SQLiteDB() as cursor:
            return cursor.execute(query).fetchone()[0]

    def update_db(pod_id, response):
        """ Stores the Ad Response in the database. """
        logging.debug('Saving Ad Response to the dai_events_table with pod_id={0}.'.format(pod_id))
        with SQLiteDB() as cursor:
            cursor.execute('UPDATE dai_events_table SET ad_response=? WHERE pod_id=?;', (response, pod_id))
    ...
    try:
        ...
        if pod_id:
            ...
            # If debug is enabled, save the XML to disk and report the Advertisement's Slot IDs and Time Position
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                ...
                for elem in tree.findall(path):
                    ...
                    time_position = str(int(float(elem.attrib.get('timePosition'))))
                    template = 'FreeWheel\'s XML Ad Response #{0} contains Slot ID:{1} and Time Position:{2}.'
                    message = template.format(pod_id, slot_id, time_position)
...
def collect_impression_info(pod_id, tree):
    ...
    for element in tree.findall(path):
        ...
        time_position = str(int(float(element.attrib.get('timePosition'))))
...
def collect_creatives(pod_id, tree):
    ...
    for element in tree.findall('ads/ad'):
        list_of_ad_creatives.append({
            ...
            'creative_duration': int(float(element.find(path).attrib.get('duration'))),
...
def fill_impression_table(data):
    ...
    with SQLiteDB() as cursor:
        ...
        if logging.getLogger().isEnabledFor(logging.DEBUG):
            ...
            table = '\n'.join(map(str, rows))
            message = 'Number of Impressions in FreeWheel\'s Ad Response #{0}: {1}\n{2}\n{3}'
            logging.debug(message.format(pod_id, len(rows), column_names, table))
    ...
    try:
        ...
        with SQLiteDB() as cursor:
            ...
            cursor.executemany('''
                ...
                WHERE ad_id=:ad_adId AND type='quartile'  
            ''', data)
            ...
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                ...
                rows = cursor.fetchall()
                table = '\n'.join(map(str, rows))
                message = 'Number of Creatives in FreeWheel\'s Ad Response #{0}: {1}\n{2}\n{3}'
                logging.debug(message.format(pod_id, len(rows), column_names, table))
                print()
        ...
        script = '''
            ...
            CREATE TABLE IF NOT EXISTS 
                creative_table (
                    ...
                    ad_adUnit INTEGER NOT NULL,
                    creative_adUnit TEXT NOT NULL,
                    creative_baseUnit TEXT NOT NULL,
                    creative_creativeId INTEGER NOT NULL,
                    creative_duration INTEGER NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS 
                qvt_table (
                    slot_id TEXT NOT NULL DEFAULT 'Missing',
                    afid TEXT NOT NULL DEFAULT 'Missing',
                    asnw TEXT NOT NULL DEFAULT 'Missing',
                    caid TEXT NOT NULL DEFAULT 'Missing',
                    channel TEXT NOT NULL DEFAULT 'Missing',
                    nw TEXT NOT NULL DEFAULT 'Missing',
                    ssnw TEXT NOT NULL DEFAULT 'Missing',
                    prof TEXT NOT NULL DEFAULT 'Missing',
                    emcr TEXT NOT NULL DEFAULT 'Missing',
                    esvt TEXT NOT NULL DEFAULT 'Missing',
                    exvt TEXT NOT NULL DEFAULT 'Missing',
                    qtcb TEXT NOT NULL DEFAULT 'Missing',
                    slcb TEXT NOT NULL DEFAULT 'Missing',
                    tpcl TEXT NOT NULL DEFAULT 'Missing', 
                    anchor_time TEXT NOT NULL DEFAULT 'Missing',
                    allow_seeking TEXT NOT NULL DEFAULT 'Missing',
                    method TEXT NOT NULL DEFAULT 'Missing',
                    start_offset REAL NOT NULL DEFAULT 0,
                    stop_offset REAL NOT NULL DEFAULT 0,
                    playback_delay TEXT NOT NULL DEFAULT 'Missing',
                    start_offset_with_delay REAL NOT NULL DEFAULT 0,
                    duration REAL NOT NULL DEFAULT 0,
                    time_position REAL NOT NULL DEFAULT 0,
                    title TEXT NOT NULL DEFAULT 'Missing',
                    url TEXT NOT NULL DEFAULT 'Missing',
                    vod TEXT NOT NULL DEFAULT 'Missing',
                    log TEXT NOT NULL DEFAULT 'Missing'
                ); 
                CREATE UNIQUE INDEX idx_qvt_slot_id ON qvt_table (slot_id);
...
def print_database():
    ...
    with SQLiteDB() as cursor:
        ...
        if logging.getLogger().isEnabledFor(logging.DEBUG):
            ...
            rows = cursor.fetchall()
            table = '\n'.join(map(str, rows))
            message = 'Total Number of Records in the Database: {0}\n{1}\n{2}'
            logging.debug(message.format(len(rows), column_names, table))
            print()
...
def db_catalog_http_status(beacon):
    ...
    with SQLiteDB() as cursor:
        if beacon.is_valid:
            cursor.execute('UPDATE impression_table SET http_status=? WHERE pk=?;', (beacon.response_code, beacon.pk))
        else:
            logging.warning('Invalid Beacon. Unable to catalog in the database: {0}'.format(vars(beacon)))


def populate_template(pod_id):
    # type: (object) -> None
    """
    Use the data from FreeWheel's XML Ad Response to prepare a template in another dictionary (e.g., journal) with
    the expected Advertisement Slot ID (e.g., slotImpression), Advertisement IDs (e.g., Quartiles), and 3rd-party
...
def sm3_handle_points(match):
    """
    Process the SM3PointsCacheItem.  Each time we encounter the SM3CachePointsItem, we compare it against the last one
    seen.  If they are identical, then we update the existing entry so we have a recent timestamp.  Otherwise, we save
    the current SM3PointsCacheItem so we can compare it against subsequent ones and reference it later when needed.
...
def sm3_store_log(match):
    ...
    """
    with SQLiteDB() as cursor:
        cursor.execute("INSERT INTO dai_events_table (sm3pointscacheitem) VALUES (?);", (match.group("logline"),))
        return cursor.lastrowid


def sm3_store_ad_request(match, pod_id):
    """
    Parses the SM3PointsCacheItem object for the Advertisement Request URL submitted to FreeWheel and stores it.
    ...
    for point in points:
        if point.get('_debug'):
            ...
            if url:
                ...
                with SQLiteDB() as cursor:
                    cursor.execute("UPDATE dai_events_table SET ad_request=? WHERE pod_id=?;", (url, pod_id))
                    break


def sm3_store_ad_response(match, pod_id):
    """
    Parses the SM3PointsCacheItem object for the Advertisement Response from FreeWheel and stores it.
    ...
    for point in points:
        if point.get('_debug'):
            ...
            if vmap_response:
                ...
                with SQLiteDB() as cursor:
                    cursor.execute('UPDATE dai_events_table SET ad_response=? WHERE pod_id=?;', (xml, pod_id))
                    break


def sm3_drop_db_duplicates():
    """
    Removes the duplicate SM3PointsCacheItems for the Event Callback information stored in the database.
    """
    with SQLiteDB() as cursor:
...
def sm3_collect_info(match, pod_id):
    """
    Parse the SM3PointsCacheItem for the Event Callback information and store them in the database.
    This object contains information about all the Impression Events, such as the Advertisement
    Slot IDs (e.g., slotImpression), Advertisement IDs (e.g., Quartiles), and 3rd-party Tracking Impressions.
    ...
    # For each impression in the list of results, take keys from the dict as SQL params and execute the SQL statement.
    with SQLiteDB() as cursor:
        ...
        if logging.getLogger().isEnabledFor(logging.DEBUG):
            ...
            rows = cursor.fetchall()
            table = "\n".join(map(str, rows))
            message = "Number of Impressions in SM3\'s Ad Response #{0}: {1}\n{2}\n{3}"
            logging.debug(message.format(pod_id, len(rows), column_names, table))
...
def sm3_remove_debug_info(match):
    # type: (object) -> object
    """
    Remove the debug information from the SM3PointsCacheItem.  In order to determine if the previous and current
    SM3PointsCacheItems are identical or not, the '_debug' information needs to be removed before performing the
    comparision.
...
def swap_event_term(event):
    # type: (str) -> str
    """
    Returns the Adaptive Player's terminology with FreeWheel's terminology or vice versa.
    Example: swaps the 'slotImpression' term used by FreeWheel with 'AdBreakStartEvent'.
...
def find_key(key, dictionary):
    ...
    :param str key:
    :param collection.OrderedDict dictionary:
    :rtype: Iterator[str]
    """
...
def write_ad_response_to_file(pod_id, xml_string):
    # type: (int, str) -> None
    """
    Saves FreeWheel's SmartXML Response to a temporary directory as a JSON for debugging.
...
def write_sm3_to_file(pod_id, string):
    # type: (int, str) -> None
    """
    Saves the SM3PointsCacheItem to a temporary directory as a JSON for debugging.
...
def display_qvt_and_ad_request_parameters(data):
    """ Compare the QVT parameters to the applicable Ad Request parameters submitted to FreeWheel. """
    pd.set_option('colheader_justify', 'center')
    pd.set_option('display.expand_frame_repr', False)  # Prevent the table from line wrapping
    pd.set_option('display.max_columns', None)
...
def pandas_summarize_slot_and_quartile_metrics():
    # type: () -> None
    """ Use the pandas library to provide a metrics summary of the Slot and Quartile impressions. """

    # TODO: Look into using the records from the SQL database as a source to generate metrics from
...
def pandas_summarize_tracking_metrics():
    # type: () -> None
    """ Use the pandas library to provide a metrics summary of the Tracking impressions. """
    print('\n\n\n\n')
    print_border_long()
...
def print_pandas_groupby_summary():
    ...
    query = '''
            SELECT 
                pk, pod_id, slot_id, time_position, ad_id, event, sent, received, creative_url 
            FROM 
                impression_table 
            '''
    connection = sqlite3.connect('/tmp/sqlite::memory:?cache=shared')
...
def merge_multiple_lines(source):
    ...
    Newer releases of the Adaptive Player have log entries that span multiple lines to make them more human readable.
    This function determines if the log entry starts with a timestamp (e.g., YYYY/MM/DD HH:MM:SS), and if it doesn't,
    begins collapsing those multiple lines into one.

    :param file source: Adaptive Player's player.log file
    :param file timestamp: Regular expression for date and time
    :rtype: Iterator[str]
    """
    previous_line = ''
...
def to_html():
    ...
    # Query the database for all of the Ad Responses (pod_ids) encountered in the player.log; Convert results to a list.
    pod_ids = db_fetch_pod_ids()
    # Loop through each of the Ad Responses found in the database.
    for pod_id in pod_ids:
        # Query the database for the Ad Breaks (slot_IDs) contained with each of the Ad Responses.
        ...
        for slot_id in slot_ids:
            # Query the database for the QVT information with this slot ID; generate an empty one if necessary
...
def db_fetch_qvt_record(slot_id):
    with SQLiteDB(row=True) as cursor:
        record = cursor.execute("SELECT * FROM qvt_table WHERE slot_id=?", (slot_id,)).fetchone()
        if record is None:
            logging.warning('No QVT records found for slot_id={0}. Generating an empty set.'.format(slot_id))
            cursor.execute("INSERT INTO qvt_table (slot_id) VALUES (?);", (slot_id,))
            cursor.execute("SELECT * FROM qvt_table WHERE slot_id=?", (slot_id,))
            record = cursor.fetchone()
        return record
...
def generate_html_report(pod_id, slot_id, qvt_record):
    ...
    content += to_html_display_validate_ad_request_url(pod_id, qvt_record)
    content += to_html_display_ad_response_xml(pod_id)
    content += to_html_display_beacon_summary(pod_id, slot_id)
    content += to_html_display_beacon_validation(slot_id)
    content += to_html_display_unmatched_beacons_and_responses()
...
def to_html_display_channel_info(pod_id, slot_id, qvt_record):
    ...
    if 'Missing' in qvt_record['title']:
        title = "<font color='red'>Missing</font>"
    else:
        title = qvt_record['title']

    if 'Missing' in qvt_record['channel']:
        channel = "<font color='red'>Missing</font>"
    else:
    ...
    end_time = convert_epoch_to_datestr(qvt_record['stop_offset'])

    html = "<div class='title' id='Home'>"
    html += "DAI Validation Report for Ad Request Number: {0}<br>Slot ID: {1}<br>".format(pod_id, slot_id)
    html += "</div>"
    html += "<div class='info'>"
    html += "<div>"
    html += "Channel:<br>"
...
def to_html_display_qvt_link(qvt_record):
    ...
    """
    Displays the URL to the QVT and the contents of the entire QVT.
    ...
    timestamp = "<font color='red'>Missing</font>"
    url = 'Missing'
    if re.search(r"ad_info.*ad_breaks", logline):
...
def to_html_display_ad_request_url(pod_id):
    """
    Display the Advertisement Request submitted to FreeWheel.

    :param int pod_id: The ID of the Ad Request submitted to FreeWheel.
    :return: html
    :rtype: str
    """
    params = ['flag', 'nw', 'csid', 'caid', 'asnw', 'ssnw', 'prof', 'afid', 'fw_vcid', '_fw_vcid2', 'slid', 'tpcl',
              'tpos', 'maxd', 'mind', 'cpsq', 'ssto']

    with SQLiteDB() as cursor:
        request = cursor.execute("SELECT ad_request FROM dai_events_table WHERE pod_id=?;", (pod_id,)).fetchone()[0]
        ad_request = ParseAdRequest(request)

    html = "<hr>"
    html += "<div id='Ad Request URL'>"
    html += "<div class='subtitle'>Ad Request URL:</div><br>"
    html += "Date and Time: <span class='date_time'>{0}</span><br>".format(ad_request.timestamp)
    html += "<span class='no_wrap'>URL: {0}</span><br>".format(ad_request.url)
    ...
    for key in params:
        html += "<tr><td>{0}:</td><td><span class='code'>{1}</span></td></tr>".format(key, ad_request.params[key])
    html += "<tr><td><br></td></tr>"

    # Loop through less important parameters and exclude keys where the values are missing.
...
def to_html_display_validate_ad_request_url(pod_id, qvt_record):
    """
    Display the validated Ad Request parameters.
    :return: html
    :rtype: str
    """
...
def validate_ad_request_url(pod_id, qvt_info):
    """ Validates the Ad Request URL """
    with SQLiteDB() as cursor:
        cursor.execute("SELECT ad_request FROM dai_events_table WHERE pod_id=?;", (pod_id,))
        ad_request = ParseAdRequest(cursor.fetchone()[0])
    ...
    # TODO:
    #   May need to modify ParseAdRequest to return only the parameters associated with
    #   the given slot ID.  We need to specify an offset to identify the Slot ID's parameters.
    #   The reason for this is that when the Ad Request URL is parsed, the values are a list
    #   of values for each name.  So if the Slot IDs are [1, 2, 3] and the TPOS is [32, 45, 56],
    #   then we know that TPOS 32 is for Slot ID 1.
    key_offset = ad_request.params['slid'].index(qvt_info['slot_id'])

    # -------------------------------
    # Adapt URL
    # -------------------------------
    data['Adapt URL']['found'] = '(Not Found)'
    data['Adapt URL']['expected'] = 'http://p-adapt.movetv.com OR http://b-cg20-adapt.movetv.com'
    if 'http://p-adapt.movetv.com' in ad_request.url:
        data['Adapt URL']['found'] = 'http://p-adapt.movetv.com'
        data['Adapt URL']['expected'] = 'http://p-adapt.movetv.com'
        data['Adapt URL']['pass'] = 'True'
    elif 'http://b-cg20-adapt.movetv.com' in ad_request.url:
        data['Adapt URL']['found'] = 'http://b-cg20-adapt.movetv.com'
        data['Adapt URL']['expected'] = 'http://b-cg20-adapt.movetv.com'
        data['Adapt URL']['pass'] = 'True'
        is_production = False
    elif 'http://b-adapt.movetv.com' in ad_request.url:
        data['Adapt URL']['found'] = 'http://b-adapt.movetv.com'
        data['Adapt URL']['expected'] = 'http://b-adapt.movetv.com'
        data['Adapt URL']['pass'] = 'True'
        is_production = False
    else:
        data['Adapt URL']['pass'] = 'False'

    # -------------------------------
    # FreeWheel URL
    # -------------------------------
    data['FW URL']['found'] = '(Not Found)'
    data['FW URL']['expected'] = 'http://5d40b.s.fwmrm.net/ad/g/1 OR http://5d40a.s.fwmrm.net/ad/g/1'
    if 'http://5d40b.s.fwmrm.net/ad/g/1' in ad_request.url:
        data['FW URL']['found'] = 'http://5d40b.s.fwmrm.net/ad/g/1'
        data['FW URL']['expected'] = 'http://5d40b.s.fwmrm.net/ad/g/1'
        data['FW URL']['pass'] = 'True'
    elif 'http://5d40a.s.fwmrm.net/ad/g/1' in ad_request.url:
        data['FW URL']['found'] = 'http://5d40a.s.fwmrm.net/ad/g/1'
        data['FW URL']['expected'] = 'http://5d40a.s.fwmrm.net/ad/g/1'
        data['FW URL']['pass'] = 'True'
        is_production = False
    else:
        data['FW URL']['pass'] = 'False'

    # -------------------------------
    # FreeWheel Capabilities
    # -------------------------------
    if 'Missing' in ad_request.params['flag']:
        data['FW Capabilities']['found'] = '(Not Found)'
        data['FW Capabilities']['expected'] = 'exvt qtcb emcr slcb esvt sync rste'
    else:
        data['FW Capabilities']['found'] = ', '.join(ad_request.params['flag']).strip()
        data['FW Capabilities']['expected'] = 'exvt qtcb emcr slcb esvt sync rste'

    if data['FW Capabilities']['found'] == data['FW Capabilities']['expected']:
        data['FW Capabilities']['pass'] = 'True'
    else:
        data['FW Capabilities']['pass'] = 'False'

    # -------------------------------
    # Network ID (NW)
    # -------------------------------
    if 'Missing' in ad_request.params['nw']:
        data['NW']['found'] = '(Not Found)'
        data['NW']['expected'] = '381963'
    else:
        data['NW']['found'] = ', '.join(ad_request.params['nw'])
        data['NW']['expected'] = '381963'

    if not is_production:
        data['NW']['expected'] = '381962'

    if data['NW']['found'] == data['NW']['expected']:
        data['NW']['pass'] = 'True'
    else:
        data['NW']['pass'] = 'False'

    # -------------------------------
    # Site Section Network ID (SSNW)
    # -------------------------------
    if 'Missing' in ad_request.params['ssnw']:
        data['SSNW']['found'] = '(Not Found)'
        data['SSNW']['expected'] = '381963'
    else:
        data['SSNW']['found'] = ', '.join(ad_request.params['ssnw'])
        data['SSNW']['expected'] = '381963'

    if not is_production:
        data['SSNW']['expected'] = '381962'

    if data['SSNW']['found'] == data['SSNW']['expected']:
        data['SSNW']['pass'] = 'True'
    else:
        data['SSNW']['pass'] = 'False'

    # -------------------------------
    # FreeWheel Profile (PROF)
    # -------------------------------
    if 'Missing' in ad_request.params['prof']:
        data['PROF']['found'] = '(Not Found)'
        data['PROF']['expected'] = '381963:sling'
    else:
        data['PROF']['found'] = ', '.join(ad_request.params['prof'])
        data['PROF']['expected'] = '381963:sling'

    if not is_production:
        data['PROF']['expected'] = '381962:sling'

    if data['PROF']['found'] == data['PROF']['expected']:
        data['PROF']['pass'] = 'True'
    else:
        data['PROF']['pass'] = 'False'

    # -------------------------------
    # Minimum Duration (MIND)
    # -------------------------------
    data['MIND'] = dict()
    # Determine if we've parsed multiple values for this parameter.
    if len(ad_request.params['mind']) == 1:
        if 'Missing' in str(qvt_info['duration']):
            data['MIND']['expected'] = '(Unable to find start_offset and stop_offset in QVT used to validate mind)'
            data['MIND']['pass'] = 'Skip'
        else:
            if 'Missing' in ad_request.params['mind']:
                data['MIND']['found'] = '(Not Found)'
                data['MIND']['pass'] = 'False'
            else:
                data['MIND']['found'] = ''.join(ad_request.params['mind'])
                data['MIND']['expected'] = str(int(qvt_info['duration']))
                if abs(float(data['MIND']['found']) - float(data['MIND']['expected'])) <= 1.0:
                    data['MIND']['pass'] = 'True'
                else:
                    data['MIND']['pass'] = 'False'
    elif len(ad_request.params['mind']) > 1:
        if 'Missing' in ad_request.params['mind']:
            data['MIND']['found'] = '(Not Found)'
            data['MIND']['expected'] = str(int(qvt_info['duration']))
            data['MIND']['pass'] = 'False'
        else:
            data['MIND']['found'] = ad_request.params['mind'][key_offset]
            data['MIND']['expected'] = str(int(qvt_info['duration']))
            data['MIND']['pass'] = 'False'
            if int(ad_request.params['mind'][key_offset]) == int(qvt_info['duration']):
                data['MIND']['pass'] = 'True'
            else:
                data['MIND']['pass'] = 'False'

    # -------------------------------
    # Maximum Duration (MAXD)
    # -----------------------
    data['MAXD'] = dict()
    if len(ad_request.params['maxd']) == 1:
        if 'Missing' in str(qvt_info['duration']):
            data['MAXD']['expected'] = '(Unable to find start_offset and stop_offset in QVT used to validate maxd)'
            data['MAXD']['pass'] = 'Skip'
        else:
            if 'Missing' in ad_request.params['maxd']:
                data['MAXD']['found'] = '(Not Found)'
                data['MAXD']['expected'] = str(int(qvt_info['duration']))
                data['MAXD']['pass'] = 'False'
            else:
                data['MAXD']['found'] = ''.join(ad_request.params['maxd'])
                data['MAXD']['expected'] = str(int(qvt_info['duration']))
                if abs(float(data['MAXD']['found']) - float(data['MAXD']['expected'])) <= 1.0:
                    data['MAXD']['pass'] = 'True'
                else:
                    data['MAXD']['pass'] = 'False'
    elif len(ad_request.params['maxd']) > 1:
        if 'Missing' in ad_request.params['maxd']:
            data['MAXD']['found'] = '(Not Found)'
            data['MAXD']['expected'] = str(int(qvt_info['duration']))
            data['MAXD']['pass'] = 'False'
        else:
            data['MAXD']['found'] = ad_request.params['maxd'][key_offset]
            data['MAXD']['expected'] = str(int(qvt_info['duration']))
            if int(ad_request.params['maxd'][key_offset]) == int(qvt_info['duration']):
                data['MAXD']['pass'] = 'True'
            else:
                data['MAXD']['pass'] = 'False'

    # -------------------------------
    # Slot Time Position Class (TPCL)
    # -------------------------------
    if 'Missing' in ad_request.params['tpcl']:
        data['TPCL']['found'] = '(Not Found)'
    else:
        data['TPCL']['found'] = ad_request.params['tpcl'][key_offset]

    if is_linear:
        data['TPCL']['expected'] = 'midroll'
    else:
        if 'midroll' in data['TPCL']['found']:
            data['TPCL']['expected'] = 'midroll'
        elif 'preroll' in data['TPCL']['found']:
            data['TPCL']['expected'] = 'preroll'
        else:
            data['TPCL']['expected'] = 'midroll OR preroll'

    if data['TPCL']['found'] == data['TPCL']['expected']:
        data['TPCL']['pass'] = 'True'
    else:
        data['TPCL']['pass'] = 'False'

    # -------------------------------
    # Channel Service ID (CSID)
    # -------------------------------
    if 'Missing' in ad_request.params['csid']:
        data['CSID'] = {'found': '(Not Found)'}
    else:
        data['CSID'] = {'found': ', '.join(ad_request.params['csid'])}

    if 'Missing' in qvt_info['channel']:
        data['CSID']['expected'] = '(Unable to find channel name in the QVT used to validate csid)'
        data['CSID']['pass'] = 'Skip'
    else:
        devices = [
            'android', 'android_phone', 'android_tablet', 'android_tv', 'ios', 'ipad', 'airtvplayer',
            'roku', 'tvos'
        ]
        if is_linear:
            ending = 'live'
        ...
        data['CSID']['expected'] = \
            ...
            "| ipad " \
            "| tvos " \
            "| roku ]_' + qvt_info['channel'] + '_' + ending"
        # foo = ' '.join(['otto_', [str(dev) for dev in devices], qvt_info['channel'], '_', ending])
        possible_csids = list()
        for device in devices:
            possible_csids.append('otto_' + device + '_' + qvt_info['channel'] + '_' + ending)
        if data['CSID']['found'] in possible_csids:
            data['CSID']['pass'] = 'True'
            data['CSID']['expected'] = data['CSID']['found']
        else:
            data['CSID']['pass'] = 'False'

    # -------------------------------
    # Slot Impression ID (SLID)
    # -------------------------------
    if 'Missing' in ad_request.params['slid']:
        data['SLID']['found'] = '(Not Found)'
    else:
        data['SLID']['found'] = ad_request.params['slid'][key_offset]

    if 'Missing' in qvt_info['slot_id']:
        data['SLID']['expected'] = '(Unable to find the cue_point_number in the QVT used to validate slid)'
        data['SLID']['pass'] = 'Skip'
    else:
        data['SLID']['expected'] = qvt_info['slot_id']
        if data['SLID']['found'] == data['SLID']['expected']:
            data['SLID']['pass'] = 'True'
        else:
            data['SLID']['pass'] = 'False'

    # -------------------------------
    # CPSQ
    # -------------------------------
    if 'Missing' in ad_request.params['cpsq']:
        data['CPSQ']['found'] = '(Not Found)'
    else:
        data['CPSQ']['found'] = ad_request.params['cpsq'][key_offset]
    if 'Missing' in qvt_info['slot_id']:
        data['CPSQ']['expected'] = '(Unable to find the cue_point_number in the QVT used to validate cpsq)'
        data['CPSQ']['pass'] = 'Skip'
    else:
        data['CPSQ']['expected'] = qvt_info['slot_id']
        if data['CPSQ']['found'] == data['CPSQ']['expected']:
            data['CPSQ']['pass'] = 'True'
        else:
            data['CPSQ']['pass'] = 'False'

    # -------------------------------
    # SSTO
    # -------------------------------
    if 'Missing' in ad_request.params['ssto']:
        data['SSTO']['found'] = '(Not Found)'
    else:
        data['SSTO']['found'] = ad_request.params['ssto'][key_offset]

    if 'Missing' in qvt_info['slot_id']:
        data['SSTO']['expected'] = '(Unable to find the cue_point_number in the QVT used to validate ssto)'
        data['SSTO']['pass'] = 'Skip'
    else:
        data['SSTO']['expected'] = qvt_info['slot_id']
        if data['SSTO']['found'] == data['SSTO']['expected']:
            data['SSTO']['pass'] = 'True'
        else:
            data['SSTO']['pass'] = 'False'

    # -------------------------------
    # Time Position (TPOS)
    # -------------------------------
    if 'Missing' in ad_request.params['tpos']:
        data['TPOS']['found'] = '(Not Found)'
    else:
        data['TPOS']['found'] = ', '.join(ad_request.params['tpos'])
    if len(ad_request.params['tpos']) == 1:
        if 'Missing' in qvt_info['anchor_time']:
            data['TPOS']['expected'] = '(Unable to find the anchor_time in the QVT used to validate tpos)'
            data['TPOS']['pass'] = 'Skip'
        else:
            if 'Missing' in ad_request.params['tpos']:
                data['TPOS']['found'] = '(Not Found)'
                # data['TPOS']['expected'] = str(qvt_info['start_offset'] - float(qvt_info['anchor_time']))
                data['TPOS']['expected'] = str(int(qvt_info['time_position']))
                data['TPOS']['pass'] = 'False'
            else:
                data['TPOS']['found'] = ''.join(ad_request.params['tpos'])
                # data['TPOS']['expected'] = str(qvt_info['start_offset'] - float(qvt_info['anchor_time']))
                data['TPOS']['expected'] = str(int(qvt_info['time_position']))
                if abs(float(data['TPOS']['found']) - float(data['TPOS']['expected'])) <= 1.0:
                    data['TPOS']['pass'] = 'True'
                else:
                    data['TPOS']['pass'] = 'False'
    elif len(ad_request.params['tpos']) > 1:
        if 'Missing' in ad_request.params['tpos']:
            data['TPOS']['found'] = '(Not Found)'
            data['TPOS']['expected'] = str(qvt_info['duration'])
            data['TPOS']['pass'] = 'False'
        else:
            data['TPOS']['found'] = ad_request.params['tpos'][key_offset]
            data['TPOS']['expected'] = str(int(qvt_info['time_position']))
            # data['TPOS']['expected'] = str(qvt_info['duration'])
            if int(ad_request.params['tpos'][key_offset]) == int(qvt_info['time_position']):
                data['TPOS']['pass'] = 'True'
            else:
                data['TPOS']['pass'] = 'False'
    return data
...
def to_html_display_ad_response_xml(pod_id):
    # type: (str) -> str
    """
    Displays the XML Ad Response in HTML format so it can be displayed properly in the DAI report.

    :param int pod_id: The ID of the Ad Request submitted to FreeWheel.
    :returns: HTML formatted output for the Ad Response XML Section of the DAI Validation Report.
    :rtype: str
    """
    with SQLiteDB() as cursor:
        response = cursor.execute("SELECT ad_response FROM dai_events_table WHERE pod_id=?;", (pod_id,)).fetchone()
    if response[0] is not None:
        xml_string = response[0].split(' ', 2)[2]
        html_validate_xml = validate_xml(xml_string)
    else:
        xml_string = '''<?xml version="1.0" encoding="UTF-8"?><errors><error name="MISSING XML"/></errors>'''
    ...
    html += '{0}'.format(minidom.parseString(xml_string).toprettyxml(indent='    ').replace('    ', '&emsp;&emsp;')
                         .replace('<', '&lt;')
                         .replace('>', '&gt;')
                         .replace('\n', '<br>'))
    html += "</div></div>"
...
def to_html_display_beacon_summary(pod_id, slot_id):
    ...
    :returns: HTML formatted output for the Beacon Summary Section of the DAI Validation Report.
    :rtype: str
    """
    data = journal.dictionary['XML Index'][pod_id]['Slot ID'][slot_id]
    time_position = data['Time Position']
    ad_count = len(data['Ad References'])
    # sent = len(list(find_key("Sent", data["Ad References"])))
    # received = len(list(find_key("Received", data["Ad References"])))
    ...
    # html += "<tr><td>Number of Tracking URLs:</td><td>Sent={0} / Received={1</td></tr>".format(sent, received)
    html += "<tr><td>Total Ad Duration (secs):</td><td>{0}</td></tr>".format(fetch_slot_impression_duration(slot_id))
    html += "</table>"
    html += "</div>"
    html += "<br>"
...
def to_html_display_beacon_validation(slot_id):
    # type: (unicode) -> object
    """
    Queries the SQL database for the given Slot Impression ID and wraps the records returned in HTML so the records
    can be displayed properly in th Beacon Validation Section of the DAI report.

    :param unicode slot_id: The Slot Impression ID.
    :returns: HTML formatted output for the Beacon Validation Section of the DAI Validation Report.
    :rtype: str
    """
    count = 0
    html = "<hr><div id='Beacon Validation'><div class='subtitle'>Beacon Validation:</div><br>"
    with SQLiteDB(row=True) as cursor:
        for record in cursor.execute("SELECT * FROM impression_table WHERE slot_id=?;", (slot_id,)):
            if 'slotImpression' in record['event']:
                count = 0
                html += "<div class='indent'>SlotImpression ID : {}<br>".format(record['slot_id'])
...
def to_html_display_unmatched_beacons_and_responses():
    # type: () -> str
    """
    Queries the SQL database for the unmatched beacons and wraps the records returned in HTML so the records can be
    displayed properly in the Unmatched section of the DAI report.

    :returns: The Unmatched Beacons Section of the DAI Validation Report.
    :rtype: str
    """
    html = "<hr>"
    ...
    html += "<br>"
    with SQLiteDB(row=True) as cursor:
        cursor.execute('SELECT log FROM unmatched_table;')
        ...
        if records:
            ...
            for record in records:
                ...
                html += '''
                    ...
                    <font color='red'><span class='no_wrap'>{0} : {1}</span></font><br></div>
                '''.format(str(count).zfill(2), record['log'].encode('ascii', 'ignore'))
        else:
            html += '''<div class='indent beacon'><font color='lime'>(PASS) All Beacons Matched</font><br></div>'''
    html += "</div>"
    html += "<br>"
    html += "<hr>"
    return html
...
def to_html_beacon_event_info(record):
    ...
    :rtype: str
    """
    # <div class='indent beacon'>{overall_firing_order} : Valid Firing Sequence (Overall)<br></div>
    html = '''
       <br>
...
def to_html_tracking_event_info(record):
    ...
    :rtype: str
    """
    foo_http_status = record['http_status']
    html = '''
        <br><div class='indent'>
...
def calculate_duration_for_slot_impression():
    # type: () -> None
    """
    Calculates the duration of each slotImpression.  The total duration of a slotImpression is calculated by
    adding up the duration of each of the individual Advertisements found within the slotImpression.
    """
    with SQLiteDB(row=True) as cursor:
        ...
        for row in cursor.fetchall():
            ...
            cursor.execute('''
            UPDATE 
                impression_table SET duration = (
                    SELECT SUM(duration) 
                    FROM impression_table 
                    WHERE type='quartile' AND event='defaultImpression' AND slot_id=?
                )
            WHERE type='slot' AND event='slotImpression' AND slot_id=?''', (slot_id, slot_id))


def calculate_actual_firing_time_for_adaptive_player():
    # type: () -> None
    """
    Updates the timestamp of the "actual_firing_time" parameter. For each Impression in the database the date-time
    string at the beginning of the Adaptive Player log is converted to epoch.  Then the database record is updated
    with this timestamp to indicate when the Adaptive Player sent the Impression.
    """
    with SQLiteDB(row=True) as cursor:
        cursor.execute("SELECT pk, sent_log FROM impression_table;")
        for row in cursor.fetchall():
            pk, sent_log = row
            m = re.match(r'^\d{4}[/.-]\d{2}[/.-]\d{2}\s\d{2}:\d{2}:\d{2}[.]\d+', sent_log)
            if m:
                epoch = convert_datestr_to_epoch(m.group())
                cursor.execute("UPDATE impression_table SET actual_firing_time=? WHERE pk=?", (epoch, pk))
                logging.debug('pk={0} actual_firing_time={1} sent_log={2}'.format(pk, epoch, sent_log))


def calculate_expected_firing_time_for_slot_impression():
    # type: () -> None
    """
    Updates the timestamp of the slotImpression's "expected_firing_time" parameter.  This parameter help determines
    whether or not the Adaptive Player sent the Impression when it was supposed to.  The parameter is calculated
    when processing the Ad Break section in the QVT:
        Formula: record["start_offset_with_delay"] = record["start_offset"] + record["playback_delay"]
    """
    with SQLiteDB(row=True) as cursor:
        cursor.execute('''
            SELECT
                impression_table.pk, 
                impression_table.slot_id, 
                impression_table.expected_firing_time, 
                qvt_table.start_offset_with_delay 
            FROM 
                impression_table, qvt_table 
            WHERE
                type='slot' AND 
                event='slotImpression' AND 
                qvt_table.slot_id=impression_table.slot_id;
        ''')
        records = cursor.fetchall()
        for record in records:
            cursor.execute('''
                UPDATE 
                    impression_table 
                SET 
                    expected_firing_time=? 
                WHERE 
                    pk=?;
                ''', (record['start_offset_with_delay'], record['pk'])
                           )

            msg = 'pk={0}, slot_id={1}, expected_firing_time={2}'
            logging.debug(msg.format(record['pk'], record['slot_id'], record['start_offset_with_delay']))


def calculate_expected_firing_time_for_default_impression():
    # type: () -> None
    """
    Updates the timestamp of the Impression's "expected_firing_time" parameter.  The timestamp is used to
    determine whether or not the Adaptive Player sent the Impression when it was supposed to.

    For each slotImpression, save the time the slotImpression is scheduled to be sent.  Then query the database
    for each of the defaultImpressions contained within the slotImpression.  Each of the defaultImpressions
    represent when an individual Ad is starting and so we'll update its expected_firing_time and grab its
    Ad Duration so we can use it to compute when the next Ad or defaultImpression is scheduled to be sent.
    """
    total_ad_duration = 0
    with SQLiteDB(row=True) as cursor:
        cursor.execute("SELECT slot_id, expected_firing_time FROM impression_table WHERE event='slotImpression';")
        slot_impression_records = cursor.fetchall()
        for slot in slot_impression_records:
            start_time = slot['expected_firing_time']
            cursor.execute('''
                SELECT 
                    pk, duration 
                FROM 
                ...
                WHERE 
                    slot_id=? AND 
                    type='quartile' AND 
                    event='defaultImpression' AND 
                    expected_firing_time=0;
                ''', (slot["slot_id"],)
                           )
            default_impression_records = cursor.fetchall()
            for row in default_impression_records:
                scheduled_start_time = start_time + total_ad_duration
                cursor.execute('''
                    UPDATE 
                        impression_table 
                    SET 
                    ...
                    WHERE 
                        pk=?;
                    ''', (scheduled_start_time, row['pk'])
                               )
                total_ad_duration += row['duration']


def calculate_expected_firing_time_for_quartile_impression():
    # type: () -> None
    """
    Updates the timestamp of each of the Quartile's "expected_firing_time" parameter.  The timestamp is used to
    determine whether or not the Adaptive Player sent the Impression when it was supposed to.
    """
    events = [('firstQuartile', 0.25), ('midPoint', 0.5), ('thirdQuartile', 0.75), ('complete', 0.99)]
    with SQLiteDB(row=True) as cursor:
        # Retrieve all the defaultImpressions in the database
        cursor.execute('''
            SELECT 
                pk, 
                slot_id,
            ...
            WHERE
                type='quartile' AND 
                event='defaultImpression';
        ''')
        default_impression_records = cursor.fetchall()
        for row in default_impression_records:
            for event in events:
                # Use the time the defaultImpression is supposed to be sent to compute when each of the
                # other quartiles are supposed to be sent and update it's record in the database
                scheduled_start_time = row['expected_firing_time'] + (row['duration'] * event[1])
                cursor.execute('''
                    UPDATE 
                        impression_table 
                    SET 
                    ...
                    WHERE 
                        ...
                        ad_id=? AND
                        event=?;
                    ''', (scheduled_start_time, row['slot_id'], row['ad_id'], event[0])
                               )
                msg = 'slot_id={0}, ad_id={1}, event={2}, event_percent={3}, schedule_start_time={4}'
                logging.debug(msg.format(row['slot_id'], row['ad_id'], event[0], event[1], scheduled_start_time))


def calculate_difference_in_beacon_firing_times():
    # type: () -> None
    """
    Determines the difference in time between when the Adaptive Player was scheduled to send the Impression
    and when the Impression was actually sent.
    """
    with SQLiteDB(row=True) as cursor:
        cursor.execute("SELECT pk, sent_log, expected_firing_time, actual_firing_time FROM impression_table;")
        # For each impression that the AP sent, calculate the difference in time.
        for row in cursor.fetchall():
            pk, sent_log, expected_firing_time, actual_firing_time = row
            if sent_log:
                delta = round(actual_firing_time - expected_firing_time, 2)
            else:
                delta = 0
            cursor.execute("UPDATE impression_table SET delta_firing_time=? WHERE pk=?;", (delta, pk))
            msg = 'pk={0}, expected_firing_time={1}, actual_firing_time={2}, delta={3}, sent_log={4}'
            logging.debug(msg.format(pk, expected_firing_time, actual_firing_time, delta, sent_log))
...
def color_beacons(args):
    with SQLiteDB(row=True) as cursor:
        cursor.execute("SELECT pk, internal_log, sent_log, received_log FROM impression_table;")
        for row in cursor.fetchall():
            pk, internal, sent, received = row
            if args.html:
                if internal[:4].isdigit():
                    internal = "<font color='lime'>{0}</font>".format(internal)
                else:
                    internal = "<font color='red'>{0}</font>".format(internal)

                if sent[:4].isdigit():
                    sent = "<font color='lime'>{0}</font>".format(sent)
                else:
                    sent = "<font color='red'>{0}</font>".format(sent)

                if received[:4].isdigit():
                    received = "<font color='lime'>{0}</font>".format(received)
                else:
                    received = "<font color='red'>{0}</font>".format(received)

            if args.text:
                if internal[:4].isdigit():
                    internal = colored('{0}'.format(internal), 'green')
                else:
                    internal = colored('{0}'.format(internal), 'red')

                if sent[:4].isdigit():
                    sent = colored('{0}'.format(sent), 'green')
                else:
                    sent = colored('{0}'.format(sent), 'red')

                if received[:4].isdigit():
                    received = colored('{0}'.format(received), 'green')
                else:
                    received = colored('{0}'.format(received), 'red')

            cursor.execute(
                "UPDATE impression_table SET internal_log=?, sent_log=?, received_log=? WHERE pk=?;",
                (internal, sent, received, pk)
            )
...
def validate_beacon_fired():
    # type: () -> None
    """
    Determines if each impression in the database contains a log entry indicating the Adaptive Player sent the
    impression and it received a response.
    """
    with SQLiteDB() as cursor:
        for row in cursor.execute("SELECT pk, sent_log, received_log FROM impression_table;").fetchall():
            pk, sent_log, received_log = row
            if all([sent_log[:4].isdigit(), received_log[:4].isdigit()]):
                result = PASS
            else:
                result = FAIL
            cursor.execute("UPDATE impression_table SET beacons_found=? WHERE pk=?;", (result, pk))


def validate_beacon_firing_order():
    # type: () -> None
    """
    Queries all the records in the database and, for each beacon fired, determines if Adaptive Player reports
    the event in the correct sequence.  Specifically, does the timestamp of when the beacon was sent occur
    before the timestamp of the beacon response.  Technically, it isn't possible for Adaptive Player to
    receive a response before a beacon is sent, so we're just checking to ensure the AP is logging the
    ...
    with SQLiteDB() as cursor:
        for row in cursor.execute("SELECT pk, sent_log, received_log from impression_table;").fetchall():
            ...
            if not all([time_sent[:4].isdigit(), time_received[:4].isdigit()]):
                ...
                result = datetime.strptime(time_sent, timestampfmt) < datetime.strptime(time_received, timestampfmt)
                message = "Beacon Firing Order: key={0} result={1} sent='{2}' received='{3}'"
                logging.debug(message.format(pk, result, time_sent, time_received))
                if result:
                    result = PASS
                else:
                    result = FAIL
...
def validate_overall_beacon_firing_order():
    # type: () -> None
    """
    Queries the records in the database and for each beacon fired and determines if Adaptive Player reports
    the event in the correct sequence. Specifically, does the timestamp of when the beacon was sent occur
    before the timestamp of the beacon response.  Technically, it isn't possible for Adaptive Player to
    receive a response before a beacon is sent, so we're just checking the logging of events.
    """
    previous_event = ''
    previous_firing_time = ''
    previous_pk = ''
    with SQLiteDB(row=True) as cursor:
        for row in cursor.execute("SELECT pk, event, actual_firing_time * from impression_table;").fetchall():
...
def validate_http_response_status():
    ...
    with SQLiteDB(row=True) as cursor:
        for row in cursor.execute("SELECT pk, http_status FROM impression_table;").fetchall():
            pk, http_status = row
...
def fetch_slot_impression_duration(slot_id):
    """ Returns the slotImpression's total Ad Slot duration in seconds. """
    with SQLiteDB() as cursor:
        cursor.execute("SELECT duration FROM impression_table WHERE slot_id=? AND event='slotImpression';", (slot_id,))
...
def color_red(value):
    ...
    """
    Colors numbers in a Pandas dateframe lime green if they're positive and red if they're negative.
...
def convert_datestr_to_epoch(date_string, date_format='%Y/%m/%d %H:%M:%S.%f'):
    """ Convert Date and Time string to Epoch (time in secs). """
...
def parse_args():
    ...
    parser.add_argument('-s', '--slot', dest='slot', help='display report for this slot_id')
    parser.add_argument('-t', '--text', dest='text', help='display output in text', action='store_true')
    parser.add_argument('-v', '--version', version='%(prog)s {version}'.format(version='1.8.2'), action='version')
...
def cleanup():
    ...
    logging.info('- Removing the following database: {0}'.format(SQLiteDB().path))
...
def validate_xml(xml_string):
    ...
    def convert_to_html(data):
        ...
        html = '''
            ...
            <th>complete</th></tr>
        '''
        for slot_id, ad_ids in data.iteritems():
            html += "<tr><td valign='top' align='center'>{0}</td>".format(slot_id)
            for ad_id, quartiles in ad_ids.iteritems():
                html += "<td valign='top' align='center'>{0}".format(ad_id)
    ...
    try:
        ...
        for element in tree.findall(base_path):
            ...
            tpos = str(int(float(element.attrib.get('timePosition'))))
...
if __name__ == "__main__":
    try:
        ...
        # Some imported module is setting a logging handler and is causing logging.basicConf to be a no-op, so this
        # is a work around to clear the handler before configuring the logging level specified on the command line
        ...
        if args.html:
            color_beacons(args)
            to_html()
            # Determine if there was a request for a specific slot impression; otherwise fetch all of them.
            if args.slot:
