...
#    )
    ...
    _regex_ap_response = re.compile(
        r'^(?=(?P<logline>^.*$))'
        r'(?=.*FWBeaconPacingTask\s+External\s+Point\s+Response.*responseCode\s:\s(?P<response_code>\d+))
        r'(?=.*url\s:\s(?P<url>.*$))'
    )
...
class SQLiteDB(object):
    ...
    def __init__(self, row=False, filename=None):
        ...
        self.path = '/'.join([((sys.argv[2][::-1]).split('\\', 1))[1][::-1], filename])
...
def handle_qvt(match):
    ...
    try:
        ...
        qvt_params_list = ParseQVT(qvt_str).records
...
def handle_ad_request(match):
    ...
    try:
        ...
        with DatabaseOps() as db:
            ...
            db.catalog_request(request_obj.parsed_requests)
    except Exception as ex:
...
def save_report_to_disk(pod_id, slot_id, content, status):
    ...
    """
    try:
        template = 'Ad_Response_num_{0}_Slot_ID_{1}_{2}.html'
        #directory = tempfile.gettempdir()
        directory = ((sys.argv[2][::-1]).split('\\', 1))[1][::-1]
        filename = template.format(str(pod_id).zfill(2), slot_id, status.lower())
        filepath = '/'.join([directory, filename])
...
def save_report_to_database(pod_id=1, slot_id='1', report=None):
    ...
    if not report:
        ...
        status = 'FAIL'
    with DatabaseOps() as db:
        db.insert_report(pod_id, slot_id, report, status)
...
def to_html():
    ...
    """
    with DatabaseOps() as db:
        rows = db.fetch_ad_pod_records()
        if rows:
            for row in rows:
                pod_id, slot_id, request_str, response_str = row
                qvt_record = db.fetch_qvt_record(slot_id)
    ...
    try:
        with DatabaseOps() as db:
            ...
            for row in rows:
                ...
                qvt = db.fetch_qvt_record(slot_id)
...
def validate_information():
    ...
    # validate_overall_beacon_firing_order()
