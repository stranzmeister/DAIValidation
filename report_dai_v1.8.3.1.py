...
class ParseAdRequest:
    ...
    def _parse(self):
        ...
        expected_params = {
            # 1) Global info about URL request as a whole
            ...
            'cpsq', 'envp', 'slid', 'tpcl', 'ptgt', 'maxd', 'mind', 'slau', 'tpos',

            # 4) Site specific values
            'comscore_device', 'comscore_impl_type', 'comscore_platform',
...
def parse_qvt(logline):
    ...
    def compute_slot_duration(stop_offset_unix_ts, start_offset_unix_ts):
        """ Returns the duration of the Slot Impression in seconds. """
        if all([stop_offset_unix_ts, start_offset_unix_ts]):
            duration = float(stop_offset_unix_ts) - float(start_offset_unix_ts)
            return str(int(round(duration)))

    def compute_time_position(start_offset_unix_ts, anchor_time_unix_ts):
        ...
        if all([start_offset_unix_ts, anchor_time_unix_ts]):
            ...
            # should be calculated up to 3 decimal places
    ...
    try:
        ...
        try:
            ...
            values['emcr'] = fw_info.get('expectedMultipleCreativeRenditions', missing)
            values['esvt'] = fw_info.get('enableServerVastTranslation', missing)
            values['exvt'] = fw_info.get('recordVideoView', missing)
            values['qtcb'] = fw_info.get('requiresQuartileCallbackUrls', missing)
            values['slcb'] = fw_info.get('supportsSlotCallbacks', missing)
            ...
            if linear_info.get('anchor_time', False):
                values['content_type'] = 'live'
            else:
                values['content_type'] = 'vod'
            ...
            if isinstance(ad_breaks, dict):
                ...
                values['duration'] = compute_slot_duration(values['stop_offset'], values['start_offset'])
                ...
                list_of_qvt_records.append(values)
    ...
    if values is not None:
        with SQLiteDB() as cursor:
            cursor.executemany('''
                ...
                VALUES (
                    :slot_id, :afid, :asnw, :caid, :channel, :nw, :ssnw, :prof,
                    :emcr, :esvt, :exvt, :qtcb, :slcb, :tpcl, :anchor_time,
                    :allow_seeking, :method, :start_offset, :stop_offset,
                    :playback_delay, :start_offset_with_delay, :duration,
                    :time_position, :title, :url, :content_type, :log
                );
            ''', values)
...
def process(line):
    ...
    """
    try:
        # if line.startswith("2019/04/03 22:50:24.429827"):
    ...
    with SQLiteDB() as cursor:
        ...
        script = '''
            ...
            CREATE TABLE IF NOT EXISTS qvt_table (
                ...
                content_type TEXT NOT NULL DEFAULT 'Missing',
...
def validate_ad_request_url(pod_id, qvt_info):
    """ Validates the parameters in the Ad Request URL. """
    channel_name = qvt_info['channel']
    content_type = qvt_info['content_type']
    ...
    # TODO:
    #   Need to modify ParseAdRequest to return only the parameters associated
    #   with the given slot ID. We need to specify an offset to identify the
    #   Slot ID's parameters. The reason for this is that when the Ad Request
    #   URL is parsed, the values are a list of values for each name.  So
    #   if the Slot IDs are [1, 2, 3] and the TPOS is [32, 45, 56], then
    #   we know that TPOS 32 is for Slot ID 1.
    ...
    if 'http://p-adapt.movetv.com' in ad_request.url:
        data['Adapt URL']['found'] = data['Adapt URL']['expected'] = 'http://p-adapt.movetv.com'
        data['Adapt URL']['pass'] = 'True'
    elif 'http://b-adapt.movetv.com' in ad_request.url:
        data['Adapt URL']['found'] = data['Adapt URL']['expected'] = 'http://b-adapt.movetv.com'
        data['Adapt URL']['pass'] = 'True'
        is_production = False
    elif 'http://b-cg20-adapt.movetv.com' in ad_request.url:
        data['Adapt URL']['found'] = data['Adapt URL']['expected'] = 'http://b-cg20-adapt.movetv.com'
        data['Adapt URL']['pass'] = 'True'
        is_production = False
    elif 'movetv.com' in ad_request.url:
        m = re.match(r'^(?P<url>https?://.*movetv.com)', ad_request.url)
        data['Adapt URL']['found'] = m.group('url')
    ...
    # -------------------------------
    # Asset Fallback ID (AFID)
    # -------------------------------
    if 'Missing' in qvt_info['afid']:
        if 'Missing' in ad_request.params['afid']:
            pass
        elif 'Missing' not in ad_request.params['afid']:
            data['AFID']['found'] = '(Not Found)'
            data['AFID']['expected'] = str(ad_request.params['afid'][0])
            data['AFID']['pass'] = 'False'
    else:
        data['AFID']['found'] = str(qvt_info['afid'])
        if 'Missing' in ad_request.params['afid']:
            data['AFID']['expected'] = '(Not Found)'
        else:
            data['AFID']['expected'] = str(ad_request.params['afid'][0])
        if data['AFID']['found'] == data['AFID']['expected']:
            data['AFID']['pass'] = 'True'
        else:
            data['AFID']['pass'] = 'False'
    ...
    # TODO: Look into combining the functionality of MIND and MAXD.
    # -------------------------------
    # Minimum Duration (MIND)
    # -------------------------------
    # When calculating the MIND value from the QVT(duration), it must be
    # rounded to the nearest integer.
    # The Ad Request's MIND parameter must be an integer.
    # -------------------------------
    ...
    if len(ad_request.params['mind']) == 1:
        if 'Missing' in str(qvt_info['duration']):
            ...
            if 'Missing' in ad_request.params['mind']:
                ...
                # else:
                #     data['MIND']['pass'] = 'False'
                if data['MIND']['found'] == data['MIND']['expected']:
                    data['MIND']['pass'] = 'True'
                else:
                    data['MIND']['pass'] = 'False'
    ...
    if len(ad_request.params['maxd']) == 1:
        if 'Missing' in str(qvt_info['duration']):
            ...
            if 'Missing' in ad_request.params['maxd']:
                ...
                data['MAXD']['expected'] = str(int(qvt_info['duration']))
                if data['MAXD']['found'] == data['MAXD']['expected']:
                    data['MAXD']['pass'] = 'True'
                else:
                    data['MAXD']['pass'] = 'False'
    ...
    if 'Missing' in ad_request.params['csid']:
        ...
        data['CSID']['found'] = ad_request.params['csid'][0]

    if 'Missing' in channel_name:
        data['CSID']['expected'] = '(Unable to find channel name in the QVT used to validate csid)'
        data['CSID']['pass'] = 'Skip'
    else:
        data['CSID']['expected'] = '_'.join(['otto', device_name, channel_name, content_type])

    if data['CSID']['found'] == data['CSID']['expected']:
        data['CSID']['pass'] = 'True'
    else:
    ...
    # Time Position (TPOS)
    # -------------------------------
    qvt_tpos = float(qvt_info['time_position'])
    if qvt_tpos.is_integer():
        qvt_tpos = str(int(qvt_tpos))
    else:
        qvt_tpos = str(qvt_tpos)
    ...
    if len(ad_request.params['tpos']) == 1:
        if 'Missing' in qvt_info['anchor_time']:
            ...
            if 'Missing' in ad_request.params['tpos']:
                data['TPOS']['found'] = '(Not Found)'
                data['TPOS']['expected'] = qvt_tpos
                data['TPOS']['pass'] = 'False'
            else:
                data['TPOS']['found'] = ''.join(ad_request.params['tpos'])
                data['TPOS']['expected'] = qvt_tpos
                if abs(float(data['TPOS']['found']) - float(data['TPOS']['expected'])) == 0:
                    data['TPOS']['pass'] = 'True'
                else:
                    data['TPOS']['pass'] = 'False'
            ...
            UPDATE impression_table 
               SET expected_firing_time = (
                        SELECT start_offset_with_delay 
                          FROM qvt_table 
                         WHERE qvt_table.slot_id = impression_table.slot_id
                   ) 
...
def device_lookup_table():
    ...
    if device_info:
        if device_info['device_platform'] == 'Android TV' \
            and device_info['device_model'] == 'AIRTV PLAYER':
            result = 'airtvplayer'
        elif device_info['device_platform'] == 'Android TV':
            result = 'android_tv'
        elif device_info['device_platform'] == 'Android Phone':
            result = 'android_phone'
        elif device_info['device_platform'] == 'Android Tablet':
            result = 'android_tablet'
        elif device_info['device_platform'] == 'Roku':
            result = 'roku'
        elif device_info['device_model'] == 'Apple TV':
            result = 'tvos'
        elif device_info['device_model'] == 'iPhone':
            result = 'iphone'
        elif device_info['device_model'] == 'iPad':
            result = 'ipad'
        return result
